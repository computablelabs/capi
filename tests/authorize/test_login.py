...
def test_valid_login(w3, pk, user, test_client, cloudwatch_client):
...
def test_no_login_payload(test_client, cloudwatch_client):
    login = test_client.post('/authorize/')
    assert login.status_code == 400

def test_expired_token(w3, pk, user, test_client, cloudwatch_client):
    # use our custom user to sign a message
    user = w3.eth.accounts[10]
