...
def extract_listing_hashes(logs):
    """
    listing hashes are a universal key in the protocol. Many places we just want to extract
    a list of them from a set of logs.
    Return said list and the highest block number encountered.
    """
    to_block = 0
    hashes = []
...
def listing_hash_join(logs, candidates):
    ...
    """
    hashes, to_block = extract_listing_hashes(logs)
