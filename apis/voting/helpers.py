...
def filter_candidate_removed(from_block, arg_filters=None):
    """
    given filter create a voting contract
    ...
    v = get_voting()
    if arg_filters != None:
        filter = v.deployed.events.CandidateRemoved.createFilter(fromBlock=from_block, argument_filters=arg_filters)
    else:
